...
o=== *  yerba code                  /
    ...
    ( function ( $, undefined ) {
        ...
        _yerba.prototype = {
            ...
            // this is basically here if you want to store a copy in a database
            // add a storage method, like mongodb, redis, etc, and set it with this.set( 'storage', 'mongodb' );
            storage: undefined,
            ...
            _generateId: function () {
                ...
                while ( allYerbas.hasOwnProperty( r ) )
                    r = $.genId( 8 );
            ...
            brew: function ( arr ) {
                var newYerba = new yerba( arr ),
                    newId = this._generateId();

                newYerba._yerbaId = newId;

                allYerbas[ newId ] = newYerba;
        ...
        yerba.prototype = {
            _init: function ( arr ) {
                ...
                // set some defaults. can be overridden with this.set( key, value )
                this.opts = {
            ...
            // used internally
            _generateId: function () {
                var r = $.genId( this.opts.idLength );

                while ( this.indices[ this.opts.idName ].hasOwnProperty( r ) )
                    r = $.genId( this.opts.idLength );
            ...
            _resetIndices: function () {
                ...
                this.indices[ this.opts.idName ] = {};

                this.each( function ( yerbaKey, yerbaValue ) {
                    this.indices[ this.opts.idName ][ yerbaValue[ this.opts.idName ] ] = yerbaKey;
                });
            ...
            log: function ( obj ) {
                for ( var key in obj )
                    if ( key <= yerba.logLevel )
                        console.log( 'yerba:' + key + ':> ' + obj[ key ] );
            ...
            getDataById: function ( dataID ) {
                return this.indices[ this.opts.idName ].hasOwnProperty( dataID ) ? this[ this.indices[ this.opts.idName ][ dataID ] ] : false;
            },
                ...
                this.each( function ( yerbaKey, yerbaValue ) {
                    ...
                    if ( $.isString( c.query ) || $.isNumber( c.query ) ) {
                        ...
                        for ( var key in c.query ) {
                            // incoming long line...
                            if ( yerbaValue.data.hasOwnProperty( key ) && ( ( c.opts.caseSensitive && yerbaValue.data[ key ] === c.query[ key ] ) || ( !c.opts.caseSensitive && yerbaValue.data[ key ].toLowerCase() === c.query[ key ].toLowerCase() ) ) )
                                successes++;
            ...
            addOne: function ( data ) {
                ...
                data[ this.opts.idName ] = data.data[ this.opts.idName ] || this._generateId();

                if ( this.indices[ this.opts.idName ].hasOwnProperty( data[ this.opts.idName ] ) ) {
                    this[ this.indices[ this.opts.idName ][ data[ this.opts.idName ] ] ] = data;
                } else {
                    var nl = this.length++;

                    this[ nl ] = data;

                    this.indices[ this.opts.idName ][ data[ this.opts.idName ] ] = nl;
                }
