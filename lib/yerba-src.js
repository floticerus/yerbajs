					...
					for ( var key in arg ) {
						if ( arg[ key ] !== null ) {
							for ( var key2 in arg[ key ] ) {
								key2 !== undefined && ( first[ key2 ] = arg[ key ][ key2 ] );
							}
						}
					}
				}
	...
	( function ( $, undefined ) {
		...
		yerba.prototype = {
			...
			_reIndex: function () {
				...
				this.each( function ( yerbaMate, yerbaValue ) {
					this.indices._id[ yerbaValue._id ] = yerbaMate;
				});
		...
		if ( typeof module !== 'undefined' && module.exports )
			// probably in nodejs
			module.exports = tempYerba;

		else if ( window )
			// in a web browser
			window.yerba = tempYerba;
